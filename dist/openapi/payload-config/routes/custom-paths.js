...
export const getCustomPaths = (config, type) => {
    ...
    for (const endpoint of (config.endpoints || []).filter(endpoint => isRelevant(endpoint, type))) {
        ...
        const { summary, description = 'custom operation', responseSchema = { type: 'object' }, errorResponseSchemas = {}, queryParameters = {}, } = getEndpointDocumentation(endpoint) || {};
        if (!paths[path])
            paths[path] = {};
        const operation = {
